	 ...
	 if (metric->dmax && metric->tn > metric->dmax)
        ...
        if (gmetad_config.riemann_server) {
         ...
         if (do_summary && !xmldata->ds->dead && !xmldata->rval)
		  ...
		  if (gmetad_config.fengine_server) // if the user has specified a carbon server, send the metric to carbon as well #anuj
                     carbon_ret=write_data_to_fengine(xmldata->sourcename, xmldata->hostname, name, metricval,xmldata->source.localtime);
